...
function resolveRelianImage(r,colorId='basic'){
  ...
  // Shiny e Especial usam a própria imagem quando ela foi carregada.
  // Quando não existe uma arte exclusiva, usam a imagem Basic já sincronizada,
  // em vez de tentar abrir "shiny.png" ou "especial.png" fora da pasta do Relian.
  const synced=loaded[color]||loaded.basic||r?.imageData||'';
  if(synced)return synced;
  const raw=r?.images?.[color]||r?.images?.basic||r?.image||'';
  return isUsableDirectImageSource(raw)?raw:'';
}
